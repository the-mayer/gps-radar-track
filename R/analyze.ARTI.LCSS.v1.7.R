analyze.ARTI.LCSS.v1.7 <- function(){
  ...
  #Define some variables.
    ...
    #Cleanup
      rm(datalist)
      setnames(GPS.data, c("lon","lat","Leg.Speed.km.h","Heading.deg"),c("GPS.Lon","GPS.Lat","GPS.Speed.kmh","GPS.Heading"))
    ...
    #Make the time correct
      GPS.data$Update.Time<- as.POSIXct(GPS.data$Update.Time,format="%m-%d-%Y %H:%M:%S",tz=GPS.TimeZone)
    #Convert to GMT
  ...
  #Headings as Circular
    ARTI.data$Heading..deg.N.<-circular(ARTI.data$Heading..deg.N.,type="angles",units="degrees",template="none",modulo="2pi",rotation="clock")
